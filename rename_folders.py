...
class AddDateToName:
    ...
    def check_dir_valid(cls, folder_dir: str) -> bool:
        ...
        if not os.path.isdir(folder_dir):
            ...
            return False
        if not any(
            os.path.isdir(os.path.join(folder_dir, item))
    ...
    def rename_folders_in_locations(cls) -> list:
        ...
        for directory in cls.list_of_folders:
            ...
            for folder_name in folder_name_generator:
                ...
                folder_creation_date = datetime.fromtimestamp(
                    os.path.getctime(os.path.join(directory, folder_name))
                )
                if not folder_name.split("-")[0] == folder_creation_date.strftime(
                    "%Y%m%d"
                ):
                    new_folder_name = "{0}-{1}".format(
                        folder_creation_date.strftime("%Y%m%d"), folder_name
                    )
                    os.rename(
                        os.path.join(directory, folder_name),
                        os.path.join(directory, new_folder_name),
                    )
    ...
    def gen_folders_in_directory(folder_dir: str) -> iter:
        ...
        return (
            ...
            if os.path.isdir(os.path.join(folder_dir, folder_name))
        )
